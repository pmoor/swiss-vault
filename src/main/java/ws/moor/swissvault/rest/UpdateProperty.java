...
@Put(path = "/keys/{keyId}/secrets/{secretId}/properties/{propertyId}")
public class UpdateProperty implements RestAction {
  ...
  @Inject
  UpdateProperty(Obfuscator obfuscator, SecretRepository secretRepository, ParameterExtractor parameterExtractor,
  ...
  public JsonObject update(@RequestBody JsonObject json) throws EntityNotFoundException {
    ...
    long propertyId = parameterExtractor.getPropertyId();

    DbSecretProperty property = secretRepository.getProperty(transaction, keyId, secretId, propertyId);
    
    if (json.has("name")) {
      property.setName(EncryptedValue.fromJsonObject(json.getAsJsonObject("name")));
    }
    if (json.has("value")) {
      property.setValue(EncryptedValue.fromJsonObject(json.getAsJsonObject("value")));
    }
    
    secretRepository.update(transaction, property);
    transaction.commit();

    return property.toJsonObject(obfuscator);
  }
}

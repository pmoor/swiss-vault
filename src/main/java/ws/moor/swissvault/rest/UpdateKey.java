...
@Put(path = "/keys/{keyId}")
public class UpdateKey implements RestAction {
  ...
  @Inject
  UpdateKey(KeyRepository keyRepository, Obfuscator obfuscator, ParameterExtractor parameterExtractor,
  ...
  public JsonObject update(@RequestBody JsonObject json) {
    ...
    DbKey key = keyRepository.updateKey(transaction, userId, keyId, json);
    transaction.commit();

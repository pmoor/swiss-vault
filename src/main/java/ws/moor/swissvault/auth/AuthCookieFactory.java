...
class AuthCookieFactory {
  ...
  private static final Duration COOKIE_EXPIRATION = Duration.ofHours(1);
  ...
  public Cookie createCookie(UserId userId) {
    ...
    cookie.setPath("/");
    cookie.setMaxAge((int) COOKIE_EXPIRATION.getSeconds());
  ...
  private byte[] sign(UserId userId, Instant creationTime) {
    try {
      ...
      mac.init(hmacKey);
      return mac.doFinal(
          String.format("%s:%d", userId.asString(), creationTime.toEpochMilli()).getBytes(Charsets.UTF_8));
